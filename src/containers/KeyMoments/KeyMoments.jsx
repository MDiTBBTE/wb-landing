...
export const KeyMoments = () => {
    return <div className='container keyMoments'>
        <div className='keyMoments_inner'>
            <h2 className='keyMoments_inner_title'>Ключевые моменты</h2>
            <p className='keyMoments_inner_text'>Мы хотим, чтобы наши клиенты получали максимум результатов от нашего инструмента</p>
            <ul className='keyMoments_inner_list'>
                {KEY_MOMENTS.map((moment, idx) =>
                    <li key={`${idx.toString()}_${moment.title}`} className='keyMoments_inner_list_el'>
                        <img className='keyMoments_inner_list_el_img' src="/images/key-moments/checkbox.svg" alt=""/>
                        <div className='keyMoments_inner_list_el_info'>
                            ...
                            {typeof moment.description === 'string' ?
                                ...
                                <ul className='keyMoments_inner_list_el_info_list'>
                                    {moment.description.map( (el, idx) => <li key={idx} className='keyMoments_inner_list_el_info_list_el'>{el}</li>)}
                                </ul>
                            }
                        </div>
                    </li>)
